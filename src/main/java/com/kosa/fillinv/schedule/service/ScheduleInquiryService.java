...
public class ScheduleInquiryService { // 스케줄 조회 서비스
    ...
    // 전체 스케쥴 조회
    public List<ScheduleListResponse> getScheduleList(String loginMemberId, ScheduleSearchRequest filter) {

        // 날짜 파라미터 정제 및 타입 변환 (String -> LocalDate)
        LocalDate dateParam = null;
        if (filter.date() != null && !filter.date().isBlank() && !filter.date().equals("string")) {
            try {
                dateParam = LocalDate.parse(filter.date()); // "2026-02-20" 형식을 LocalDate로 변환
        ...
        ScheduleStatus statusParam = null;
        if (filter.status() != null && !filter.status().isBlank()) {
            try {
                statusParam = ScheduleStatus.valueOf(filter.status().toUpperCase());
            } catch (IllegalArgumentException e) {
                // 잘못된 상태값이 들어오면 null 처리하거나 예외 발생
                throw new BusinessException(ErrorCode.INVALID_SCHEDULE_STATUS);
            }
        }

        // 로그인한 사용자가 연관된 스케줄을 필터링
        List<Schedule> schedules = scheduleRepository.findAllByMemberIdWithFilter(loginMemberId, dateParam, statusParam);
        ...
        return schedules.stream()
                .map(s -> {
                    ...
                    return ScheduleListResponse.from(
                            s,
                            s.getMentorNickname(),
                            validator.getNickname(s.getMenteeId()),
                            startTime,
                            userRole
                    );
                })
