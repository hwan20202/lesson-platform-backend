...
public enum TossPaymentError {
    ...
    INVALID_API_KEY(400, "잘못된 시크릿키 연동 정보입니다."),
    INVALID_REJECT_CARD(400, "카드 사용이 거절되었습니다. 카드사 문의가 필요합니다."),
    BELOW_MINIMUM_AMOUNT(400, "신용카드는 100원 이상, 계좌는 200원 이상부터 결제 가능합니다."),
    ...
    NOT_SUPPORTED_INSTALLMENT_PLAN_CARD_OR_MERCHANT(400, "할부가 지원되지 않는 카드 또는 가맹점입니다."),
    ...
    EXCEED_MAX_MONTHLY_PAYMENT_AMOUNT(400, "당월 결제 가능금액을 초과했습니다."),
    UNAUTHORIZED_KEY(401, "인증되지 않은 시크릿 키 혹은 클라이언트 키입니다."),
    REJECT_ACCOUNT_PAYMENT(403, "잔액 부족으로 결제에 실패했습니다."),
    REJECT_CARD_PAYMENT(403, "한도초과 혹은 잔액 부족으로 결제에 실패했습니다."),
    REJECT_CARD_COMPANY(403, "결제 승인이 거절되었습니다."),
    FORBIDDEN_REQUEST(403, "허용되지 않은 요청입니다."),
    REJECT_TOSSPAY_INVALID_ACCOUNT(403, "출금이체 등록이 되어 있지 않은 계좌입니다. 계좌를 다시 등록해주세요."),
    ...
    FDS_ERROR(403, "[토스페이먼츠] 위험거래가 감지되어 결제가 제한됩니다. 발송된 문자에 포함된 링크를 통해 본인인증 후 결제가 가능합니다."),
    ...
    TossPaymentError(int statusCode, String description) {
        this.statusCode = statusCode;
        this.description = description;
    }

    public static TossPaymentError get(String name) {
    ...
    public TossPaymentErrorRecord toRecord() {
        return new TossPaymentErrorRecord(statusCode, description);
    }

    public Boolean isSuccess() {
        return switch (this) {
            case ALREADY_PROCESSED_PAYMENT -> true;
            default -> false;
        };
    }
    ...
    public record TossPaymentErrorRecord(Integer statusCode, String description) {
    }
}